...
def calculate_CF(parameters):
    ...
    The input parameters have to be in a sort of configuration:
        ...
        -The effect factors are calculated according to the user distribution, in this function we can see multiples cases for this calculation.
        -The alpha is calculated using an algorithm based on the alpha star value'''
    users_aggregation = {}
...
def create_rv(parameters_deterministic_values, parameters_uncertainty_values):
    '''This function is creating random variables depending on the type of distribution. 
    The inputs of this function are the kind of distribution and the related parameters'''
    RVs = {}
...
def unif_dist_parameters(parameters_deterministic_values, parameters_uncertainty_values):
    ''' This function is used to generate the input parameters to create a uniform random variable'''
    loc=0.
    scale=0.
    for parameter in parameters_uncertainty_values:
